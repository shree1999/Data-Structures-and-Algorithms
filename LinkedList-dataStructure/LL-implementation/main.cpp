...
class LinkedList {  // class for implementing linked list.
    Node*head;
    Node*tail;
    int totalNodes;
public:
    ...
    void append(int element) {
        ...
        if(head == NULL) {  // checking if head is null i.e. linked list is empty...
            head = newNode;
            tail = newNode;
            totalNodes++;
        }
    ...
    we will consider position(pos) = 0 means that it is the head node position. */
        ...
        if(pos == 0){ // checking for 1st position
            preAppend(element);
        }
        else if(pos >= totalNodes) { //checking for last position
            append(element);
        }
        else {
            Node *newNode = new Node;
            ...
            for(int i = 0; i < pos - 1; i++) { // going to position - 1;
                prev = prev->link;
            }
